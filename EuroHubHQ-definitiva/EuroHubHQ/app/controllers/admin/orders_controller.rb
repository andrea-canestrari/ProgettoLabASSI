class Admin::OrdersController < AdminController
  ...
  def index
    @not_completed_pagy, @not_completed_orders= pagy(Order.where(completed: false).order(created_at: :asc))
    @completed_pagy, @completed_orders = pagy(Order.where(completed: true).order(created_at: :asc), page_param: :page_completed)
  end
  ...
  private
    ...
    def admin_order_params
      params.require(:order).permit(:email, :completed, :total, :address)
    end
end
